$(document).ready(function(){
			...
			success : function(response){
				...
				$.each(resp.message, function(index, value){
					brandHTML += '<tr>'+
									'<td></td>'+
									'<td>'+ value.B_Name +'</td>'+
									'<td><a class="btn btn-sm btn-info edit-brand"><span style="display:none;">'+JSON.stringify(value)+'</span><i class="fas fa-pencil-alt"></i></a>&nbsp;<a bid="'+value.B_ID+'" class="btn btn-sm btn-danger delete-brand"><i class="fas fa-trash-alt"></i></a></td>'+
								'</tr>';
				});
	...
	$(document.body).on("click", ".edit-brand", function(){
		...
		console.log(brand);
		$("input[name='e_brand_title']").val(brand.B_Name);
		$("input[name='brand_id']").val(brand.B_ID);
